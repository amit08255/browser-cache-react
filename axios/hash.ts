...
SHA256.setArray = (target, source) => {
    if (typeof Uint8Array !== 'undefined') {
        ...
        for (let i = source.length; i < target.length; i += 1) {
            target[i] = 0;
        }
    }
};
...
SHA256.digest = (message) => {
    ...
    const m = SHA256.Uint8Array(byteLength);
    ...
    const words = SHA256.Int32Array(wordLength);
    let byteIndex = 0;
    ...
    const hash = SHA256.Uint8Array(32);
    for (let i = 0; i < 4; i += 1) {
